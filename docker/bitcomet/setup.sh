...
cmd_setup() {
    ...
    local port="${BITCOMET_WEBUI_PORT:-$DEFAULT_PORT}"
    ...
    echo "  Username: ${WEBUI_USERNAME:-admin}"
    echo "  Password: ${WEBUI_PASSWORD:-admin}"
    echo
    echo -e "${BLUE}Ports:${NC}"
    echo "  WebUI: ${port}"
    echo "  BitTorrent: ${BITCOMET_BT_PORT:-6882}"
    echo
    echo -e "${BLUE}Management:${NC}"
    echo "  ./setup.sh start       Start ${SERVICE_NAME}"
    echo "  ./setup.sh stop        Stop ${SERVICE_NAME}"
    echo "  ./setup.sh restart     Restart ${SERVICE_NAME}"
...
cmd_start() {
    ...
    local host_ip; host_ip=$(get_host_ip)
    print_success "${SERVICE_NAME} started at http://${host_ip}:${BITCOMET_WEBUI_PORT:-$DEFAULT_PORT}"
}
