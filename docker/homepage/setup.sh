...
cmd_setup() {
    ...
    local port="${HOMEPAGE_PORT:-$DEFAULT_PORT}"
    ...
    echo "  Edit services.yaml, widgets.yaml, bookmarks.yaml, settings.yaml"
    echo "  Changes are picked up automatically (no restart needed)"
    echo
...
cmd_start() {
    ...
    local host_ip; host_ip=$(get_host_ip)
    print_success "${SERVICE_NAME} started at http://${host_ip}:${HOMEPAGE_PORT:-$DEFAULT_PORT}"
}
