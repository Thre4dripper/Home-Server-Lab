...
cmd_setup() {
    ...
    local port="${JELLYFIN_PORT:-$DEFAULT_PORT}"
...
cmd_start() {
    ...
    local host_ip; host_ip=$(get_host_ip)
    print_success "${SERVICE_NAME} started at http://${host_ip}:${JELLYFIN_PORT:-$DEFAULT_PORT}"
}
...
cmd_shell() {
    ...
    docker compose exec jellyfin bash || docker compose exec jellyfin sh
}
