...
cmd_setup() {
    ...
    local port="${FILEBROWSER_PORT:-$DEFAULT_PORT}"

    print_status "Starting ${SERVICE_NAME}..."
    docker compose up -d
    wait_for_service "$port" || true

    # Try to extract initial password from logs
    echo
    local password
    password=$(docker compose logs "$CONTAINER_NAME" 2>/dev/null | grep -oP 'password: \K.*' | tail -1) || true
    if [[ -n "$password" ]]; then
        ...
        echo -e "${BLUE}Login Credentials:${NC}"
        echo "  Username: admin"
        echo "  Password: (check logs: ./setup.sh logs)"
    fi
...
cmd_start() {
    ...
    local host_ip; host_ip=$(get_host_ip)
    print_success "${SERVICE_NAME} started at http://${host_ip}:${FILEBROWSER_PORT:-$DEFAULT_PORT}"
}
