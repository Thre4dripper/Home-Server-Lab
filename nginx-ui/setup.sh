...
cmd_setup() {
    ...
    local admin_port="${ADMIN_PORT:-$DEFAULT_PORT}"
    local host_ip; host_ip=$(get_host_ip)
    ...
    docker compose up -d
    wait_for_service "$admin_port" || true

    echo
    print_success "Setup complete!"
    echo
    echo -e "${BLUE}Access Information:${NC}"
    echo "  Admin UI: http://${host_ip}:${admin_port}"
    echo
    echo -e "${BLUE}Default Admin Credentials:${NC}"
    echo "  Email:    admin@example.com"
    echo "  Password: changeme"
    echo "  (You will be forced to change these on first login)"
    echo
...
cmd_start() {
    ...
    local host_ip; host_ip=$(get_host_ip)
    print_success "${SERVICE_NAME} started at http://${host_ip}:${ADMIN_PORT:-$DEFAULT_PORT}"
}
