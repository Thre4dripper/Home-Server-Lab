...
setup_directories() {
    ...
    if [[ -f ".env" ]]; then
        ...
        if [[ -n "$PUID" && -n "$PGID" ]]; then
            ...
            sudo chown -R "$PUID:$PGID" config/ data/ cache/ 2>/dev/null || {
                print_warning "Could not set ownership. You may need to run: sudo chown -R $PUID:$PGID config/ data/ cache/"
            }
        fi
    fi
}
...
show_service_info() {
    ...
    if [[ -f ".env" ]]; then
        source .env
        port=${NETDATA_PORT:-$DEFAULT_PORT}
    fi
    ...
    echo -e "  • Real-time system monitoring (1-second granularity)"
    echo -e "  • Zero-configuration monitoring out of the box"
    echo -e "  • CPU, Memory, Disk, Network monitoring"
    echo -e "  • Docker container monitoring"
    echo -e "  • Process monitoring and alerting"
    echo -e "  • Web-based dashboard with interactive charts"
    echo -e "  • Built-in alerting and notifications"
    echo
    
    echo -e "${GREEN}Key Differences from Grafana:${NC}"
    echo -e "  • ${BLUE}Real-time${NC} vs Historical data"
    echo -e "  • ${BLUE}Zero-config${NC} vs Custom dashboards"
    echo -e "  • ${BLUE}System focus${NC} vs Multi-source data"
    echo -e "  • ${BLUE}Lightweight${NC} vs Feature-rich"
    echo
    
    echo -e "${GREEN}Next Steps:${NC}"
    echo -e "  1. Access the dashboard using the URLs above"
    echo -e "  2. Explore real-time system metrics"
    echo -e "  3. Configure alerts in the web interface"
    echo -e "  4. Optional: Connect to Netdata Cloud for remote access"
    echo
}
...
show_usage() {
    ...
    echo "  setup              Set up and start Netdata (default)"
    ...
    echo
    echo "Examples:"
    echo "  $0                    # Setup and start Netdata"
